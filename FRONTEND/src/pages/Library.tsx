...
import apiClient, { showApiError, type Playlist, type MediaItem } from '../api/apiClient'
...
export default function Library() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        showApiError('Không tải được thư viện. Vui lòng kiểm tra kết nối mạng.', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      ...
      showApiError('Không tạo được playlist. Vui lòng thử lại.', err)
    } finally {
      setCreating(false)
    }
  }
