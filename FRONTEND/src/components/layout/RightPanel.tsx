    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalContent}>
        <div style={styles.modalHeader}>
          <h3 style={{ margin: 0, fontSize: 18, color: '#fff' }}>Chia sẻ bài hát</h3>
          <button style={styles.closeModalBtn} onClick={onClose}><CloseIcon /></button>
        </div>
        ...
        <div style={styles.userList}>
          {isLoading && <div style={{ color: '#b3b3b3', textAlign: 'center', fontSize: 14 }}>Đang tìm kiếm...</div>}
          {!isLoading && users.length === 0 && query.trim() !== "" && (
            <div style={{ color: '#b3b3b3', textAlign: 'center', fontSize: 14 }}>Không tìm thấy người dùng.</div>
          )}
          ...
          {!isLoading && users.map(user => (
            <div key={user.id} style={styles.userRow}>
              ...
              <button style={styles.sendBtn} onClick={() => handleShareToUser(user.id, user.name)}>
                Gửi
              </button>
            </div>
          ))}
...
export default function RightPanel({ onClose }: { onClose: () => void }) {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      previousState ? await apiClient.media.unfavorite(trackId) : await apiClient.media.favorite(trackId);
      toggleFavId(trackId);
    } catch (error) {
      setIsFavorite(previousState);
      showApiError(error);
  ...
  if (!currentTrack) {
    return (
      <div style={styles.container}>
        ...
        <div style={{...styles.body, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#b3b3b3'}}>Chưa có bài hát đang phát</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div style={styles.container}>
        <div style={styles.header}>
          <button style={styles.activeTab}>Đang phát</button>
          <button style={styles.closeBtn} onClick={onClose} title="Thu gọn"><CloseIcon /></button>
        </div>

        <div style={styles.body}>
          {currentTrack.mediaStyle === 1 ? (
            <video ref={(el) => { mediaRef.current = el; }} controls src={mediaSrc} style={styles.imageCover} />
          ) : (
            <>
          ...
          <div style={styles.trackInfo}>
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
              <button style={styles.iconBtn} onClick={() => setIsShareModalOpen(true)} title="Chia sẻ"><ShareIcon /></button>
              <button style={{ ...styles.iconBtn, color: isFavorite ? '#1DB954' : '#b3b3b3' }} onClick={handleToggleFavorite} title="Yêu thích">
                <svg width="24" height="24" viewBox="0 0 24 24" fill={isFavorite ? "#1DB954" : "none"} stroke="currentColor" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
              </button>
            </div>
          </div>
      ...
      {isShareModalOpen && <ShareModal track={currentTrack} onClose={() => setIsShareModalOpen(false)} />}
    </>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  trackTitle: { fontSize: 18, fontWeight: 700, color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  trackArtist: { fontSize: 14, color: '#b3b3b3', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  iconBtn: { background: 'transparent', border: 'none', cursor: 'pointer', color: '#b3b3b3', transition: '0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 4 },
  ...
  modalContent: { backgroundColor: '#282828', width: '400px', maxWidth: '90%', borderRadius: '8px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px', boxShadow: '0 8px 24px rgba(0,0,0,0.5)' },
  modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  closeModalBtn: { background: 'transparent', border: 'none', color: '#b3b3b3', cursor: 'pointer', padding: 4 },
  searchInput: { width: '100%', padding: '12px 16px', borderRadius: '4px', border: '1px solid transparent', backgroundColor: '#3e3e3e', color: '#fff', fontSize: '14px', outline: 'none', boxSizing: 'border-box' },
  userList: { display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '300px', overflowY: 'auto' },
  userRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid #3e3e3e' },
  userInfo: { display: 'flex', alignItems: 'center', gap: '12px' },
  userAvatar: { width: 36, height: 36, borderRadius: '50%', backgroundColor: '#535353', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 },
  userName: { color: '#fff', fontSize: '14px', fontWeight: 600 },
  sendBtn: { backgroundColor: '#1DB954', color: '#000', border: 'none', padding: '6px 16px', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold', cursor: 'pointer' }
};