...
function PlaylistLink({ id, name, isFavorite, active }: { id: number; name: string; isFavorite?: boolean; active: boolean }) {
  ...
  return (
    ...
    >
      <div style={{
        ...
        flexShrink: 0, color: isFavorite ? '#1DB954' : '#b3b3b3',
...
export default function Sidebar({ collapsed, onToggle, onCollapse }: {
  collapsed?: boolean;
  onToggle?: () => void;
  onCollapse?: () => void;
}) {
  ...
  const isActivePlaylist = (id: number) => location.pathname === `/playlist/${id}`
            ...
            <div style={{ marginTop: 2 }}>
              <Tooltip label="Những bài hát yêu thích của bạn">
                <Link to="/playlist/1" style={{ textDecoration: 'none' }}>
                  <span style={{ fontSize: 18, margin: '2px 0', cursor: 'pointer' }}>💚</span>
                </Link>
              </Tooltip>
            </div>
              ...
              >
                ...
                <button
                  style={{
                    ...
                    ...(showCollapseBtn ? styles.collapseMaskButtonVisible : {}),
                  }}
                  className="btn-icon"
...
const styles: Record<string, React.CSSProperties> = {
  ...
  collapsedPlaylistList: {
    ...
    marginTop: 4, gap: 2, flex: 1, overflowY: 'auto',
  },
  expandBtn: {
    position: 'absolute', top: 2, right: -10,
    backgroundColor: '#1a1a1a', padding: 2, borderRadius: '50%',
    fontSize: 10, color: '#b3b3b3', border: 'none', cursor: 'pointer',
