...
const searchSuggestions = [
  'Nhạc trẻ Việt Nam', 'V-Pop Hot 2026', 'Chill Lo-fi', 'Workout Mix',
  'Sơn Tùng M-TP', 'Đen Vâu', 'Hà Anh Tuấn', 'Hoàng Thùy Linh',
  'Ballad buồn', 'Rap Việt mới', 'EDM Party', 'Acoustic Guitar',
  'Nhạc không lời', 'Karaoké tình yêu', 'Top Hits Quốc Tế', 'K-pop Dance',
  'US-UK Chart', 'Nhạc phim hay', 'Video clip mới', 'Playlist học tập',
]
...
function HomeIcon() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3L2 12h3v9h6v-6h2v6h6v-9h3L12 3z"/></svg>
}
function SearchIcon() {
...
function UploadIcon() {
  ...
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M5 15h4v6h6v-6h4l-7-7-7 7z"/></svg>
}
function BellIcon({ active }: { active: boolean }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? '#fff' : 'currentColor'}><path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6V11c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/></svg>
}
function FriendsIcon({ active }: { active: boolean }) {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill={active ? '#fff' : 'currentColor'}><path d="M16 11c1.66 0 2.99-1.34 2.99-3S17.66 5 16 5c-1.66 0-3 1.34-3 3s1.34 3 3 3zm-8 0c1.66 0 2.99-1.34 2.99-3S9.66 5 8 5C6.34 5 5 6.34 5 8s1.34 3 3 3zm0 2c-2.33 0-7 1.17-7 3.5V19h14v-2.5c0-2.33-4.67-3.5-7-3.5zm8 0c-.29 0-.62.02-.97.05 1.16.84 1.97 1.97 1.97 3.45V19h6v-2.5c0-2.33-4.67-3.5-7-3.5z"/></svg>
}
function ShareIcon() {
  return <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M18 16.08c-.76 0-1.44.3-1.96.77L8.91 12.7c.05-.23.09-.46.09-.7s-.04-.47-.09-.7l7.05-4.11c.54.5 1.25.81 2.04.81 1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3c0 .24.04.47.09.7L8.04 9.81C7.5 9.31 6.79 9 6 9c-1.66 0-3 1.34-3 3s1.34 3 3 3c.79 0 1.5-.31 2.04-.81l7.12 4.16c-.05.21-.08.43-.08.65 0 1.61 1.31 2.92 2.92 2.92s2.92-1.31 2.92-2.92-1.31-2.92-2.92-2.92z"/></svg>
}
...
function saveSearchHistory(history: string[]): void {
  try {
    ...
    localStorage.setItem('tunevault:searchHistory', JSON.stringify(history.slice(0, 20)))
  } catch {
...
function addSearchHistory(query: string): void {
  ...
  if (!trimmed) return
  const next = [trimmed, ...loadSearchHistory().filter(item => item !== trimmed)].slice(0, 20)
    ...
    >
      ...
      {show && (
        <div style={{
          ...
          zIndex: 999, boxShadow: '0 4px 12px rgba(0,0,0,0.5)',
          marginTop: 4,
        }}>
          {label}
        </div>
...
export default function TopHeader({
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      ...
      if (isSearchOverlayOpen && searchFrameRef.current && !searchFrameRef.current.contains(e.target as Node)) {
  ...
  const getVisibleSuggestions = () => {
    ...
    const fromSuggestions = searchSuggestions.filter(item => !keyword || item.toLowerCase().includes(keyword))
  ...
  return (
    <div style={styles.header}>
      ...
      <div style={styles.searchSection}>
        <div style={styles.searchBar}>
          <button
            ...
            onClick={() => { openSearchOverlay(); runSearch() }}
            title="Tìm kiếm"
          >
            <SearchIcon />
          </button>
      ...
      {/* Phải: Bell, Share, Friends, Avatar (Profile) */}
      <div style={styles.rightSection}>
        <Tooltip label="Thông báo">
          <button
            className="btn-icon"
            onClick={onBellClick}
            style={{ color: bellActive ? '#fff' : '#b3b3b3', backgroundColor: bellActive ? '#282828' : 'transparent' }}
          >
        ...
        <Tooltip label="Hoạt động bạn bè">
          <button
            ...
            style={{ color: friendsActive ? '#fff' : '#b3b3b3', backgroundColor: friendsActive ? '#282828' : 'transparent' }}
          >
        ...
        <div ref={menuRef} style={{ position: 'relative' }}>
          <Tooltip label="Hồ sơ">
            <button
              style={{ ...styles.avatarBtn, ...(profileActive ? { boxShadow: '0 0 0 2px #fff' } : {}) }}
          ...
          {showProfileMenu && (
            <div style={styles.dropdown}>
              <button style={styles.dropdownItem} onClick={() => { onProfileClick(); setShowProfileMenu(false) }}>
                Hồ sơ
              </button>
      ...
      {isSearchOverlayOpen && (
        <div style={styles.searchOverlay} onMouseDown={e => {
              ...
              <div style={styles.popularGrid}>
                {popularSearches.map(item => (
                  ...
                  >
                    ...
                    <span style={styles.popularCardDesc}>{item.description}</span>
                  </button>
                ))}
              </div>
            </div>
...
const styles: Record<string, React.CSSProperties> = {
  header: {
    display: 'grid',
    gridTemplateColumns: 'minmax(220px, 1fr) minmax(320px, 640px) minmax(220px, 1fr)',
    alignItems: 'center', gap: 16,
    padding: '8px 16px', backgroundColor: '#000000',
    height: 64, flexShrink: 0,
  },
  leftSection: { display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, justifySelf: 'end' },
  // Logo thu nhỏ để vừa với header, hiển thị đầy đủ logo mà không che khuất nút.
  logoImg: {
    height: 32, width: 'auto', cursor: 'pointer',
    flexShrink: 0, borderRadius: 6, objectFit: 'contain',
  },
  ...
  searchBar: {
    ...
    backgroundColor: '#1a1a1a', borderRadius: 500,
    padding: '6px 16px',
  },
  ...
  suggestionsBox: {
    ...
    padding: 6, zIndex: 80, boxShadow: '0 12px 30px rgba(0,0,0,0.7)',
    maxHeight: 320, overflowY: 'auto',
  },
  suggestionItem: {
    width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
    gap: 8, backgroundColor: 'transparent', border: 'none',
    color: '#fff', padding: '8px 10px', borderRadius: 6,
    cursor: 'pointer', textAlign: 'left', fontSize: 13,
  },
  removeHistoryBtn: {
    width: 22, height: 22, borderRadius: '50%', border: 'none',
    backgroundColor: 'transparent', color: '#b3b3b3',
    cursor: 'pointer', fontSize: 16, lineHeight: 1,
  },
  emptySuggestion: { color: '#b3b3b3', fontSize: 13, padding: '8px 10px' },
  // --------------------------------------------------
  rightSection: { display: 'flex', alignItems: 'center', gap: 6, justifySelf: 'start' },

  avatarBtn: {
    width: 32, height: 32, borderRadius: '50%',
    backgroundColor: '#a060e8', border: 'none', color: '#fff',
  ...
  dropdown: {
    ...
    minWidth: 180, padding: 6, zIndex: 50,
